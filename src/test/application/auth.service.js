...
describe("AuthService - Pruebas Unitarias", () => {
  ...
  test("register - Debería registrar un nuevo usuario", async () => {
    ...
    // act
    const result = await authService.register(userData);// Ejecutamos la función que queremos probar, en este caso el método de registro del servicio de autenticación.

    // assert
    expect(mockUserRepository.findByEmail).toHaveBeenCalledWith(userData.email);// Verificamos que el método del repositorio se haya llamado con el argumento correcto
    expect(mockUserRepository.save).toHaveBeenCalled();// Verificamos que el método del repositorio se haya llamado para guardar el usuario
    expect(result.message).toBe("User registered successfully");// Verificamos que el resultado sea el esperado, en este caso que el usuario se haya registrado exitosamente
  });
